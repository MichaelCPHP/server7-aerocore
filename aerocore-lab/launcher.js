...
const SERVER_ROOT = path.resolve(PLATFORM_DIR, '..');
let PORT_BASE = 9000;
try { PORT_BASE = JSON.parse(fs.readFileSync(path.join(SERVER_ROOT, 'server.json'), 'utf8')).portBase || 9000; } catch (e) {}
const PORT = PORT_BASE + 200;
    ...
    for (const name of fs.readdirSync(INSTANCES_DIR)) {
      ...
      try {
        ...
        instances.push({
          ...
          port: config.port || (PORT_BASE + 210),
          frontendPort: (config.port || (PORT_BASE + 210)) + 70,
...
function startInstance(id) {
  ...
  const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
  const port = config.port || (PORT_BASE + 210);

  const proc = spawn('node', [CORE_SERVER], {
    cwd: dir,
    env: { ...process.env, LAB_INSTANCE: dir, LAB_PORT: String(port), GENESIS_DIR, GLOBAL_PLUGINS_DIR, GLOBAL_SKILLS_DIR },
...
<title>AeroCore Lab Platform</title>
